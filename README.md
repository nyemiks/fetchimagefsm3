...
* [`src/fetchimagefsm3/`](src/fetchimagefsm3/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/fetchimagefsm3/core.cljs): contains the SPA entry point, `init`
* [`test/fetchimagefsm3/`](test/fetchimagefsm3/): test files (ClojureScript,
...
5. Clone this repo and open a terminal in the `fetchimagefsm3` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/fetchimagefsm3/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns fetchimagefsm3.example
  (:require [fetchimagefsm3.config :as config])
